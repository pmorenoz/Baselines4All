...
# Load Data --
data_london = sio.loadmat('../data/london.mat')
print(data_london)
...
kernel = RBF()
likelihood = Gaussian(fit_noise=True)
model = SVGP(kernel, likelihood, M**2, input_dim=2, batch_rate=batch_rate)
...
zy = np.linspace(my - 0.2, my + 0.3, M)
zx = np.linspace(mx - 0.3, mx + 0.2, M)
...
optimizer = torch.optim.Adam([{'params': model.q_m, 'lr': lr_m},
                              ...
                              {'params': model.likelihood.parameters(), 'lr': lr_hyp},
...
plt.scatter(x[:, 0], x[:, 1], c=y_plot.flatten(), marker='o', cmap='winter', vmin=0.45, vmax=0.65, alpha=0.5,
            lw=0.5, s=100)
...
plt.plot(r'Type of contract -- Regression task')
