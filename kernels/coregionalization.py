...
class LMC(Kernel):
    ...
    def __init__(self, kernels, output_dim, rank=1, W=None, kappa=None, variance=None, length_scale=None, input_dim=None):
        ...
        if W is None:
            #self.W = torch.nn.Parameter(0.5*torch.randn(self.output_dim, self.Q, self.rank)/(self.rank ** 0.5), requires_grad=False)
            self.W = torch.nn.Parameter(torch.randn(self.output_dim, self.Q), requires_grad=True)
            #self.W = torch.nn.Parameter(torch.eye(self.Q), requires_grad=False)
        else:
            assert W.shape == (self.output_dim, self.Q, self.rank)

        # if kappa is None:
        #     self.kappa = torch.nn.Parameter(0.5*torch.ones(self.output_dim))
        # else:
        #     assert kappa.shape == (self.output_dim, )

        # Registration of coregionalization parameters
        self.register_parameter('coregionalization_W', self.W)
        # self.register_parameter('coregionalization_kappa', self.kappa)
    ...
    def B_coefficients(self):
        ...
        for q in range(self.Q):
            B_q = torch.mm(self.W[:,q:q+1], self.W[:,q:q+1].t()) #+ torch.diag(self.kappa)
            B_coeff.append(B_q)
        return B_coeff
